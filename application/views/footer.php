<footer class="footer">
    <div class="container-fluid">
        <nav>                        
            <p class="copyright text-center"><b>©
                <script>
                    document.write(new Date().getFullYear())
                </script></b>
                <a href="<?=base_url()?>">Body Effect</a>, todos los derechos reservados.
            </p>
        </nav>
    </div>
</footer>
...
<script>
    ...
    for( var i = 0; i<data.length; i++){
      ...
      console.log(data[i]['suma_semana_cobros']);
    }
	...
	$("#reimprimirForm").on('submit', function(e){
		...
		$.ajax({
			type: 'POST',
			url: '<?=base_url()?>index.php/Clientes/reimprimir_ticket',
